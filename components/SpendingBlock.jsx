...
// Import the SpendingType from the types folder.
import { SpendingType } from '@/types'

// Import the Colors used in teh project from the Colors file in the constants folder.
import Colors from '@/constants/Colors'

// Import the Icons for this component from the constants folder.
import { AirbnbIcon, AmazonIcon, DollarIcon, FigmaIcon, NetflixIcon, ShoppingCartIcon, SpotifyIcon, } from "@/constants/Icons";
  ...
  return (
    <View style={styles.spendingSectionWrapper}>
      <Text style={styles.sectionTitle}>
        Monthly <Text style={{ fontWeight: "700" }}>Spending</Text>
      </Text>

{/* For this case we will not use the FlatList as the items are displayed vertically. This is because the main HomeScreen has
the vertical ScrollView.
Instead we will use the map function to map through the data(items) and display the icon, and expenses. */}
      ...
      {spendingList.map((item) => {
        if (item.name == "AirBnB Rent") {
          ...
          icon = (
            <ShoppingCartIcon width={22} height={22} color={Colors.white} />
          );
        }
        return (
